...
profileRouter.post("/profiles", async (req, res) => {
  try {
    const role = req.body.role ? req.body.role : "plebian";
...
profileRouter.put(
  ...
  async (req, res) => {
    ...
    if (
      profileToUpdate.userName === req.session.userName ||
      req.session.role === "admin"
    ) {
